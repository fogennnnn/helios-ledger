...
import { createHash, createPrivateKey, createPublicKey, generateKeyPairSync, sign as nodeCryptoSign, verify as nodeCryptoVerify } from 'node:crypto';

// ── Sync SHA-256 (avoids async CPU limit in Merkle tree building) ───────
...
// ── Crypto ─────────────────────────────────────────────────────────────
// sha256hex is now synchronous via node:crypto
...
async function buildMerkleProof(db, targetIdx) {
  ...
  // Use synchronous hash to stay within CPU time limits
  const levels = [leaves];
  let cur = leaves;
  while (cur.length > 1) {
    const next = [];
    for (let i = 0; i < cur.length; i += 2) {
      next.push(sha256sync(cur[i] + (i + 1 < cur.length ? cur[i+1] : cur[i])));
    }
    levels.push(next);
    cur = next;
  }
...
async function handleCreateAccount(req, env, origin) {
  ...
  const tokHash  = sha256hex(rawToken);
  ...
  const contentHash  = sha256hex(acctEvent);
...
async function handleSubmitRecord(req, env, origin) {
  ...
  try {
    ...
    root2 = mp.root; proof2 = mp.proof;
  } catch(e) { return errR('E:merkle:' + e.message, 500, origin); }
  try {
    await env.DB.prepare("UPDATE ledger_meta SET value=? WHERE key='root'").bind(root2).run();
  } catch(e) { return errR('E:rootupd:' + e.message, 500, origin); }
  return jsonR({
    id, content_hash: contentHash, signature,
    signing_algorithm: 'Ed25519',
    user_verified,
    merkle_index: leafIdx, merkle_root: root2, merkle_proof: proof2,
