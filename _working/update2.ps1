# update2.ps1 - Onboarding + pulse update
...
<section id="get-started" aria-labelledby="gs-heading">
  <div class="container">
    ...
    <div class="demo-box">
      ...
      <p class="demo-note">This demo runs in your browser using the Web Crypto API — no server involved. Real deployments use Ed25519 server-side signing with a persistent Merkle tree.</p>
    </div>
  </div>
</section>
...
<script>
  ...
  // ── Browser demo using Web Crypto SHA-256 ──
  document.getElementById('demo-run').addEventListener('click', async () => {
    ...
    if (!text) { out.innerHTML = '<span style="color:var(--red)">⚠ Please enter some text first.</span>'; out.classList.add('visible'); return; }
    ...
    // Simulate a Merkle leaf index and a fake sig (real signing is server-side Ed25519)
    const leafIndex = Math.floor(Math.random() * 10000);
    const fakeSig   = hashHex.split('').reverse().join('').substring(0, 64) + hashHex.substring(0, 64);
    const ts        = new Date().toISOString();

    out.innerHTML = `<span class="ok">✓ record created (browser simulation)</span>
<br>
<span style="color:var(--muted)">content_hash  </span><span class="hash">${hashHex}</span>
<span style="color:var(--muted)">timestamp     </span>${ts}
<span style="color:var(--muted)">merkle_leaf   </span>#${leafIndex}
<span style="color:var(--muted)">signature     </span><span class="hash">${fakeSig}</span>
<span style="color:var(--muted)">model         </span>not specified
...
git commit -m 'feat: add onboarding section + browser demo + teal pulse'
git push

Write-Host 'Live at https://ai.oooooooooo.se in ~30s' -ForegroundColor Green