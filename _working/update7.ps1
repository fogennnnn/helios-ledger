# update7.ps1 - Fix SyntaxError in terminal JS
  ...
  (() => {
    const SCENES = [
      { label:'get token', lines:[
        {t:'cmt', s:'# Step 1: create an account to get your token'},
      ...
      { label:'submit', lines:[
        {t:'cmt', s:'# Step 2: seal AI content on the ledger'},
      ...
      { label:'verify', lines:[
        {t:'cmt', s:'# Step 3: verify any record is untampered'},
    ...
    function makeOutSpan(line) {
      const span = document.createElement('span');
      const cls  = line.hl ? 't-hl' : 't-val';
      const txt  = esc(line.s)
...
';
    ...
    function makeCmtSpan(line) {
      const span = document.createElement('span');
      span.innerHTML = '<span class="t-comment">' + esc(line.s) + '</span>
';
      return span;
    }

    function makeConSpan(line) {
      const span = document.createElement('span');
      let txt = esc(line.s)
        .replace(/(-[A-Za-z])/g, '<span class="t-flag">$1</span>')
...
';
    ...
    async function runScene(idx) {
      ...
      for (let li = 0; li < SCENES[idx].lines.length; li++) {
        if (stopFlag) return;
        const line  = SCENES[idx].lines[li];
        const delay = line.d || 80;
...
'), cursor);
          ...
          screen.insertBefore(makeCmtSpan(line), cursor);
          await sleep(60);

        } else if (line.t === 'con') {
          screen.insertBefore(makeConSpan(line), cursor);
          await sleep(40);
          ...
          screen.insertBefore(wrap, cursor);
          for (let ci = 0; ci <= line.s.length; ci++) {
            if (stopFlag) return;
...
'), cursor);
          ...
          await sleep(delay);
          if (stopFlag) return;
          const el = makeOutSpan(line);
          screen.insertBefore(el, cursor);
          if (line.hl) {
            const hl = el.querySelector('.t-hl');
    ...
    function startScene(idx) {
      ...
      setTimeout(() => runScene(idx), 80);
    }

    tabs.forEach((tab, i) => tab.addEventListener('click', () => startScene(i)));
    if (replay) replay.addEventListener('click', () => startScene(current));

    // auto-advance once on load
    (async () => {
      for (let i = 0; i < SCENES.length; i++) {
        startScene(i);
        await sleep(1200 + SCENES[i].lines.length * 260);
      }
    })();
  })();
...
git commit -m 'fix: terminal JS rewritten with const/let, no duplicate declarations'
git push
