# update6.ps1 - Fix terminal: token flow + no $ on continuations
# Run from C:\Users\fogen\HELIOS

Write-Host 'Deploying terminal fix...' -ForegroundColor Cyan
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .term-header {
      ...
      padding: 0.55rem 1rem; background: var(--bg3);
      border-bottom: 1px solid var(--border2);
    }
    ...
    .term-replay {
      ...
      padding: 0.1rem 0.4rem; border-radius: 4px; transition: all 0.15s;
    }
    ...
    .term-screen {
      ...
      word-break: break-all; margin: 0; color: var(--white);
    }
    .term-cursor {
      color: var(--teal);
    ...
    .t-hl {
      ...
      animation: flash-hl 0.7s ease forwards;
    }
    @keyframes flash-hl {
      0%   { background: rgba(45,212,170,0.3); border-radius: 3px; }
...
<nav aria-label="Main navigation">
  <div class="nav-inner">
    ...
    <ul class="nav-links" role="list">
      ...
      <li><a href="https://github.com/fogennnnn/helios-ledger" class="nav-cta" rel="noopener">GitHub ↗</a></li>
    </ul>
  </div>
</nav>
...
<header class="hero" role="banner">
  <div class="container">
    ...
    <h1>Prove where your<br/><span>AI content</span> came from</h1>
    <p class="hero-sub">
    ...
    <div class="hero-actions">
      <a href="https://github.com/fogennnnn/helios-ledger" class="btn btn-primary" rel="noopener noreferrer">
    ...
    <div class="term-wrap">
      <div class="term-header">
        ...
        <div class="term-tabs">
          <button class="term-tab active" data-scene="0">get token</button>
          <button class="term-tab" data-scene="1">submit</button>
...
<script>
  ...
  // ---- Animated terminal ----
  (function()    var SCENES = [
      { label:'get token', lines:[
        {t:'cmt', s:'# Step 1: create an account, get your token'},
        {t:'cmd', s:'curl -X POST https://ai.oooooooooo.se/api/accounts'},
        {t:'con', s:'     -H "Content-Type: application/json"'},
        {t:'con', s:'     -d '{"username":"alice"}''},
        {t:'gap'},
        {t:'out', s:'{'},
        {t:'out', s:'  "id": "f3a1b2c4-...",',      d:70},
        {t:'out', s:'  "username": "alice",',         d:60},
        {t:'out', s:'  "token": "9f3c1a2b4d5e...",', hl:true, d:60},
        {t:'out', s:'  "created_at": "2026-03-15T19:15:00Z"', d:60},
        {t:'out', s:'}', d:60},
        {t:'gap'},
        {t:'cmt', s:'# Save that token. Use it in every request.'}
      ]},
      { label:'submit', lines:[
        {t:'cmt', s:'# Step 2: submit AI content for provenance'},
        {t:'cmd', s:'curl -X POST https://ai.oooooooooo.se/api/records'},
        {t:'con', s:'     -H "Authorization: Bearer 9f3c1a2b4d5e..."'},
        {t:'con', s:'     -H "Content-Type: application/json"'},
        {t:'con', s:'     -d '{"content":"AI output text","model":"claude-sonnet-4"}''},
        {t:'gap'},
        {t:'out', s:'{'},
        {t:'out', s:'  "id": "d4f2a1b3-9c8e-...",',          hl:true, d:70},
        {t:'out', s:'  "content_hash": "a3f1e2d4b5c6...",',  hl:true, d:70},
        {t:'out', s:'  "signature": "9f3c1a2b4d5e...",',      d:70},
        {t:'out', s:'  "merkle_index": 4,',                   hl:true, d:70},
        {t:'out', s:'  "merkle_root": "00a1b2c3d4e5...",',    hl:true, d:70},
        {t:'out', s:'  "timestamp": "2026-03-15T19:16:02Z"',  d:70},
        {t:'out', s:'}', d:70}
      ]},
      { label:'verify', lines:[
        {t:'cmt', s:'# Step 3: verify any record by its id'},
        {t:'cmd', s:'curl https://ai.oooooooooo.se/api/records/d4f2a1b3.../verify'},
        {t:'gap'},
        {t:'out', s:'{'},
        {t:'out', s:'  "record_id": "d4f2a1b3-...",',  d:65},
        {t:'out', s:'  "valid": true,',                hl:true, d:65},
        {t:'out', s:'  "content_hash": "a3f1e2d4...",', d:65},
        {t:'out', s:'  "merkle_index": 4,',             hl:true, d:65},
        {t:'out', s:'  "merkle_proof": [...],',         d:65},
        {t:'out', s:'  "timestamp": "2026-03-15T19:16:02Z"', d:65},
        {t:'out', s:'}', d:65}
      ]}
    ]; ];
    ...
    function makeLine(line) {
      if (line.t === 'gap') return '\n';
      var span = document.createElement('span');
      if (line.t === 'cmt') {
        span.innerHTML = '<span class="t-comment">' + line.s.replace(/</g,'&lt;') + '</span>\n';
        return span;
      }
      if (line.t === 'cmd') {
      ...
      if (line.t === 'con') {
        // continuation line — no $ prompt, indented
        var txt = line.s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
        txt = txt.replace(/(-[A-Za-z])/g,'<span class="t-flag">$1</span>');
        txt = txt.replace(/(&#39;[^&#]*&#39;|&quot;[^&]*&quot;|"[^"]*")/g,'<span class="t-str">$1</span>');
        span.innerHTML = '  <span class="t-cmd">' + txt + '</span>\n';
        return span;
      }
      if (line.t === 'out') {
        var cls = line.hl ? 't-hl' : 't-val';
        var txt = line.s
          .replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
          .replace(/"([^"]+)":/g,'<span class="t-key">"$1":</span>')
          .replace(/: (true|false)/g,': <span class="t-ok">$1</span>');
        span.innerHTML = '<span class="' + cls + '">' + txt + '</span>\n';
        return span;
      }
      return null;
    }
    ...
    async function runScene(idx) {
      ...
      screen.innerHTML = '';
      if (cursor) { cursor.style.display = 'inline'; screen.appendChild(cursor); }
      ...
      for (var li = 0; li < scene.lines.length; li++) {
        ...
        if (line.t === 'gap') {
          screen.insertBefore(document.createTextNode('\n'), cursor || null);
          await sleep(100);
          continue;
        }
        if (line.t === 'cmt') {
          var el = makeLine(line);
          if (el) screen.insertBefore(el, cursor || null);
          await sleep(60);
          continue;
        }
        if (line.t === 'con') {
        ...
        if (line.t === 'cmd') {
          ...
          wrap.innerHTML = '<span class="t-prompt">$ </span>';
          var txtSpan = document.createElement('span');
          txtSpan.className = 't-cmd';
          wrap.appendChild(txtSpan);
          screen.insertBefore(wrap, cursor || null);
          for (var ci = 0; ci <= line.s.length; ci++) {
            if (stopFlag) break;
            txtSpan.textContent = line.s.slice(0, ci);
            await sleep(20 + Math.random()*18);
          }
          screen.insertBefore(document.createTextNode('\n'), cursor || null);
          await sleep(li === 0 ? 180 : 30);
          continue;
        }
        if (line.t === 'out') {
          await sleep(delay);
          ...
          var el = makeLine(line);
          if (el) screen.insertBefore(el, cursor || null);
          // re-trigger animation for highlights
          if (line.hl && el) {
            var hl = el.querySelector('.t-hl');
            if (hl) { hl.style.animation='none'; hl.offsetHeight; hl.style.animation=''; }
          }
          continue;
        }
      }
    ...
    function startScene(idx) {
      ...
      setTimeout(function(){ runScene(idx); }, 80);
    }

    tabs.forEach(function(tab, i){
      tab.addEventListener('click', function(){ startScene(i); });
    });
    if (replay) {
      replay.addEventListener('click', function(){ startScene(current); });
    }

    // auto-advance through all scenes once
    async function autoPlay() {
      for (var i = 0; i < SCENES.length; i++) {
        startScene(i);
        var waitMs = 1200 + SCENES[i].lines.length * 250;
        await sleep(waitMs);
      }
    }
...
git commit -m 'fix: terminal shows token flow, no dollar sign on continuation lines'
git push
