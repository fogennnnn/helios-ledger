...
Write-Host "`n[3] CORS omits ACAO header for unknown origin (fix #8)"
# CORS is browser-enforced. The server correctly omits Access-Control-Allow-Origin
# for unknown origins. A browser blocks the response; PowerShell does not (by design).
$bad = Invoke-WebRequest "$BASE/health" -Headers @{ "Origin" = "https://evil.com" } -UseBasicParsing
$hasACAO = $bad.Headers.ContainsKey("Access-Control-Allow-Origin")
Check "no ACAO header for evil.com" (-not $hasACAO)
try {
    $opt = Invoke-WebRequest "$BASE/health" -Method OPTIONS -Headers @{ "Origin" = "https://evil.com" } -UseBasicParsing -ErrorAction Stop
    Check "OPTIONS hard-blocked for evil.com" ($opt.StatusCode -eq 403)
} catch {
    Check "OPTIONS hard-blocked for evil.com" ($_.Exception.Response.StatusCode.value__ -eq 403)
}
